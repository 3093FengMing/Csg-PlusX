...
public class Fwmain extends JavaPlugin implements Listener {
    ...
    public void LoadTec() throws IOException {
        ...
        this.saveResource("src/main/resources/Csg-Plus.zip", true);
        File zip = new File(getDataFolder(), "src/main/resources/Csg-Plus.zip");
        ZipUtils.decompress("./plugins", zip.getAbsolutePath());
        zip.delete();

        this.saveResource("src/main/resources/CustomGoTec.zip", true);
        zip = new File(getDataFolder(), "src/main/resources/CustomGoTec.zip");
    ...
    private void SendToData() {
        ...
        data = new File(getDataFolder(), "src/main/resources/Data.yml");
        option = new File(getDataFolder(), "src/main/resources/Option.yml");
    ...
    private void Playeruuid(EntityDamageByEntityEvent evt) {
        if (evt.getDamager() instanceof Player && evt.getEntity() instanceof LivingEntity) {
            ...
            if (plist.contains(pl)) {
                plist.remove(pl);
                pl.sendMessage("UUID: " + evt.getEntity().getUniqueId());
            }
        }
    }
